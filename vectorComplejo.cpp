...
const complejo& VectorComplejo::vectorget(int index) const {
    if (index < 0 || index >= size) { throw std::out_of_range("vectorget: indice fuera de rango"); }
    return a[index];
}
void VectorComplejo::vectorset(int index, const complejo& value) {
    if (index < 0 || index >= size) { throw std::out_of_range("vectorset: indice fuera de rango"); }
    a[index] = value;
}
...
void VectorComplejo::vectorinsert(int index, const complejo& value) {
    if (index < 0 || index > size) { throw std::out_of_range("vectorinsert: indice invalido (0..size)"); }
    if (size == capacity) { vectorresize(); }
...
void VectorComplejo::vectorremove(const complejo& value) {
    ...
    if (found) {
        for (int j = pos; j < size - 1; ++j) { a[j] = a[j + 1]; }
        --size;
    }
}
...
VectorComplejo VectorComplejo::operator+(int s) const {
    ...
    r.size = size;
    for (int i = 0; i < size; ++i) { r.a[i] = a[i] + complejo(s, 0.0); }
    return r;
}
VectorComplejo VectorComplejo::operator-(int s) const {
    VectorComplejo r(capacity);
...
VectorComplejo VectorComplejo::operator/(int s) const {
    if (s == 0) { throw std::runtime_error("division por cero"); }
    VectorComplejo r(capacity);
    r.size = size;
    for (int i = 0; i < size; ++i) { r.a[i] = a[i] / complejo(s, 0.0); }
    return r;
...
VectorComplejo VectorComplejo::operator+(const complejo& c) const {
    VectorComplejo r(capacity);
    r.size = size;
    for (int i = 0; i < size; ++i) { r.a[i] = a[i] + c; }
    return r;
}
VectorComplejo VectorComplejo::operator-(const complejo& c) const {
    VectorComplejo r(capacity);
