...
bool solveABC(const MatrizComplejo& A, const VectorComplejo& c, VectorComplejo& sol) {
    ...
    if (!(A.getRows() == c.vectorgetsize())) { return false; }
...
bool saveSolutionToTxt(const std::string& path,
    ...
    if (!(labels.size() == sol.vectorgetsize())) { return false; }
...
// ----------- guardar BIN -----------
bool saveSolutionToBin(const std::string& path,
                       const VectorIncognitas& labels,
                       const VectorComplejo& sol) {
    if (!(labels.size() == sol.vectorgetsize())) { return false; }
    ...
    while (i < n && ok) {
        ...
        if (ok) { fb.write(reinterpret_cast<const char*>(&re), sizeof(re)); }
        if (fb.good()) { fb.write(reinterpret_cast<const char*>(&im), sizeof(im)); }
        if (!fb.good()) { ok = false; }
...
bool solveFromTxtAndSave(const std::string& inPathTxt,
                         const std::string& outPathTxt,
                         const std::string& outPathBin) {
    MatrizComplejo A;
...
bool solveFromBinAndSave(const std::string& inPathBin,
                         const std::string& outPathTxt,
                         const std::string& outPathBin) {
    MatrizComplejo A;
    VectorIncognitas labels;
