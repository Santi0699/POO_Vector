...
// ------- Util -------
static complejo toC(int v)
{
    complejo z(static_cast<double>(v), 0.0);
    return z;
}
...
complejo MatrizComplejo::get(int r, int c) const
{
    ...
    {
        throw std::out_of_range("MatrizComplejo::get");
    }
    return data[idx(r, c)];
}
void MatrizComplejo::set(int r, int c, const complejo &v)
{
    if (r < 0 || c < 0 || r >= rows || c >= cols)
    {
        throw std::out_of_range("MatrizComplejo::set");
    }
    data[idx(r, c)] = v;
}
...
complejo MatrizComplejo::detRec() const
{
    if (rows != cols)
    {
        throw std::runtime_error("det: no cuadrada");
    }
    if (rows == 0)
    {
        complejo one(1.0, 0.0);
        return one;
    }
    if (rows == 1)
    {
...
MatrizComplejo MatrizComplejo::adjugate() const
{
    if (rows != cols)
    {
        throw std::runtime_error("adj: no cuadrada");
    }
...
MatrizComplejo MatrizComplejo::inverse() const
{
    if (rows != cols)
    {
        throw std::runtime_error("inv: no cuadrada");
    }
    complejo det = determinant();
    complejo cero(0.0, 0.0);
    bool isZero = (det == cero);
    if (isZero)
    {
        throw std::runtime_error("inv: det=0");
    }
    MatrizComplejo adj = adjugate();
    MatrizComplejo inv(rows, cols);
    int r = 0;
    while (r < rows)
    {
        int c = 0;
...
MatrizComplejo MatrizComplejo::operator+(const MatrizComplejo &b) const
{
    ...
    {
        throw std::runtime_error("sum cplx dims");
    }
    ...
    while (r < rows)
    {
        ...
        while (cc < cols)
        {
            c.set(r, cc, get(r, cc) + b.get(r, cc));
            ++cc;
        }
        ++r;
    }
...
MatrizComplejo MatrizComplejo::operator-(const MatrizComplejo &b) const
{
    if (!(rows == b.rows && cols == b.cols))
    {
        throw std::runtime_error("sub cplx dims");
    }
    MatrizComplejo c(rows, cols);
    int r = 0;
    while (r < rows)
...
MatrizComplejo MatrizComplejo::operator*(const MatrizComplejo &b) const
{
    if (cols != b.rows)
    {
        throw std::runtime_error("mul cplx dims");
    }
...
MatrizComplejo operator+(const MatrizInt &a, const MatrizComplejo &b)
{
    ...
    {
        throw std::runtime_error("sum mix dims");
    }
    ...
    while (r < b.getRows())
    {
        ...
        while (cc < b.getCols())
        {
            c.set(r, cc, toC(a.get(r, cc)) + b.get(r, cc));
            ++cc;
        }
        ++r;
    }
...
MatrizComplejo operator-(const MatrizInt &a, const MatrizComplejo &b)
{
    if (!(a.getRows() == b.getRows() && a.getCols() == b.getCols()))
    {
        throw std::runtime_error("sub mix dims");
    }
    MatrizComplejo c(b.getRows(), b.getCols());
    int r = 0;
    while (r < b.getRows())
...
MatrizComplejo operator-(const MatrizComplejo &a, const MatrizInt &b)
{
    ...
    while (r < a.getRows())
    {
        ...
        while (cc < a.getCols())
        {
            c.set(r, cc, a.get(r, cc) - toC(b.get(r, cc)));
            ++cc;
        }
        ++r;
    }
...
MatrizComplejo operator*(const MatrizInt &a, const MatrizComplejo &b)
{
    ...
    {
        throw std::runtime_error("mul mix dims");
    }
    ...
    while (i < a.getRows())
    {
        ...
        while (j < b.getCols())
        {
            ...
            while (k < a.getCols())
            {
                acc = acc + (toC(a.get(i, k)) * b.get(k, j));
                ++k;
            }
            c.set(i, j, acc);
            ++j;
...
MatrizComplejo operator*(const MatrizComplejo &a, const MatrizInt &b)
{
    ...
    while (i < a.getRows())
    {
        ...
        while (j < b.getCols())
        {
            ...
            while (k < a.getCols())
            {
                acc = acc + (a.get(i, k) * toC(b.get(k, j)));
                ++k;
            }
            c.set(i, j, acc);
            ++j;
...
MatrizComplejo operator/(const MatrizComplejo &a, const MatrizInt &b)
{
    ...
    int i = 0;
    while (i < binvI.getRows())
    {
        int j = 0;
        while (j < binvI.getCols())
        {
            binv.set(i, j, toC(binvI.get(i, j)));
            ++j;
        }
        ++i;
    }
...
{
    if (A.getCols() != x.vectorgetsize())
    {
        throw std::runtime_error("A*x cplx dims");
    }
    ...
    while (i < A.getRows())
    {
        ...
        while (k < A.getCols())
        {
            acc = acc + (A.get(i, k) * x.vectorget(k));
            ++k;
        }
        y.vectoraddback(acc);
        ++i;
...
{
    if (xT.vectorgetsize() != A.getRows())
    {
        throw std::runtime_error("x^T*A cplx dims");
    }
...
VectorComplejo operator*(const MatrizComplejo &A, const vector &x)
{
    if (A.getCols() != x.vectorgetsize())
    {
        throw std::runtime_error("A*x mix dims");
    }
    VectorComplejo xc(A.getCols());
    int i0 = 0;
    while (i0 < x.vectorgetsize())
    {
        xc.vectoraddback(complejo(x.vectorget(i0), 0.0));
        ++i0;
    }
    return A * xc;
}
VectorComplejo operator*(const vector &xT, const MatrizComplejo &A)
{
    ...
    int i0 = 0;
    while (i0 < xT.vectorgetsize())
    {
        xTc.vectoraddback(complejo(xT.vectorget(i0), 0.0));
        ++i0;
    }
    return xTc * A;
}
