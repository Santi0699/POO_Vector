...
int MatrizInt::get(int r, int c) const
{
    ...
    {
        throw std::out_of_range("MatrizInt::get");
    }
    return data[idx(r, c)];
}
void MatrizInt::set(int r, int c, int v)
{
    if (r < 0 || c < 0 || r >= rows || c >= cols)
    {
        throw std::out_of_range("MatrizInt::set");
    }
    data[idx(r, c)] = v;
}
...
long long MatrizInt::detRec() const
{
    if (rows != cols)
    {
        throw std::runtime_error("determinant: no cuadrada");
    }
    if (rows == 0)
    {
        return 1;
...
long long MatrizInt::determinant() const 
{ 
    return detRec(); 
}
...
MatrizInt MatrizInt::adjugate() const
{
    if (rows != cols)
    {
        throw std::runtime_error("adjugate: no cuadrada");
    }
...
MatrizInt MatrizInt::inverse() const
{
    if (rows != cols)
    {
        throw std::runtime_error("inverse: no cuadrada");
    }
    long long det = determinant();
    if (det == 0)
    {
        throw std::runtime_error("inverse: det=0");
    }
...
MatrizInt MatrizInt::operator+(const MatrizInt &b) const
{
    ...
    {
        throw std::runtime_error("sum: dims");
    }
...
MatrizInt MatrizInt::operator-(const MatrizInt &b) const
{
    if (!(rows == b.rows && cols == b.cols))
    {
        throw std::runtime_error("sub: dims");
    }
    MatrizInt c(rows, cols);
    int r = 0;
    while (r < rows)
...
MatrizInt MatrizInt::operator*(const MatrizInt &b) const
{
    if (cols != b.rows)
    {
        throw std::runtime_error("mul: dims");
    }
    ...
    while (i < rows)
    {
        ...
        while (j < b.cols)
        {
            ...
            while (k < cols)
            {
                acc += static_cast<long long>(get(i, k)) * b.get(k, j);
                ++k;
            }
...
vector operator*(const MatrizInt &A, const vector &x)
{
    ...
    {
        throw std::runtime_error("A*x dims");
    }
    vector y(A.getRows());
    int i = 0;
    while (i < A.getRows())
    {
        ...
        while (k < A.getCols())
        {
            acc += static_cast<long long>(A.get(i, k)) * x.vectorget(k);
            ++k;
        }
        y.vectoraddback(static_cast<int>(acc));
        ++i;
    }
    return y;
}
vector operator*(const vector &xT, const MatrizInt &A)
{
    ...
    {
        throw std::runtime_error("x^T*A dims");
    }
    vector y(A.getCols());
    int j = 0;
    while (j < A.getCols())
    {
        ...
        while (k < A.getRows())
        {
            acc += static_cast<long long>(xT.vectorget(k)) * A.get(k, j);
            ++k;
        }
        y.vectoraddback(static_cast<int>(acc));
        ++j;
